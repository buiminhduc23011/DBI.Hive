...
export const TaskSearch: React.FC<TaskSearchProps> = ({ onTaskSelect }) => {
    ...
    const [osModifier, setOsModifier] = useState<string>('Ctrl');
        ...
        <div ref={containerRef} className="relative" data-onboarding="search-task">
            ...
            <div className="relative">
                ...
                <input
                    ...
                    role="combobox"
                    className="w-full md:w-64 pl-10 pr-8 py-2 bg-gray-100 dark:bg-gray-700 border border-transparent focus:border-dbi-primary focus:bg-white dark:focus:bg-gray-600 rounded-lg text-sm dark:text-white transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-dbi-primary"
                />
                {!searchText && !isOpen && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none hidden md:block">
                        <kbd className="hidden sm:inline-flex items-center gap-1 px-1.5 font-mono text-[10px] font-medium text-gray-500 bg-gray-200 dark:bg-gray-600 dark:text-gray-400 rounded">
                            {osModifier} K
                        </kbd>
                    </div>
                )}
